...
function getTopic(obj){
	...
	var i=obj.id;
	$('.topicimg'+i).css('background','url(/images/menu/topicbg1'+i+'.png) no-repeat');
	$(".topicimg"+i).next().css('color','#fff');
}
function outTopic(obj){
	var i=obj.id;
	if(mouse!=i){
	obj.className="litopic";
	$('.topicimg'+i).css('background','url(/images/menu/topicbg0'+i+'.png) no-repeat');
	$(".topicimg"+i).next().css('color','#333');
	}
}
